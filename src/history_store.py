...
class HistoryStore:
    ...
    def save_turn(self, session_id: str, turn_number: int, user_input: str, assistant_response: str):
        """
        保存一轮对话
    ...
    def get_session_history(self, session_id: str, limit: Optional[int] = None) -> List[Tuple]:
        ...
        if limit:
            ...
            LIMIT ?
            """
            cursor.execute(query, (session_id, limit * 2))  # *2 因为每轮有用户和助手两条
        else:
            query = """
            SELECT turn_number, role, content, timestamp, created_at
            FROM conversation_history
    ...
    def get_recent_history(self, session_id: str, n_turns: int = 3) -> str:
        ...
        history_sorted = sorted(history, key=lambda x: (x[0], 0 if x[1] == 'user' else 1))
